...
def train(train_loader, device, net, criterion, optimizer):
    ...
    loss_iter_train = []
    ssim_iter_train = []
    for idx_iter, (data, label, LFPSF, data_info) in tqdm(enumerate(train_loader), total=len(train_loader), ncols=70):
        ...
        out = net(data, LFPSF, data_info)
        loss = criterion(data, out, LFPSF, data_info)
...
def test(test_loader, device, net, criterion, optimizer, save_dir=None):
    ...
    psnr_iter_test = []
    ssim_iter_test = []
    for idx_iter, (Lr_SAI_y, Hr_SAI_y, Sr_SAI_cbcr, LFPSF, data_info, LF_name) in tqdm(enumerate(test_loader), total=len(test_loader), ncols=70):

        [Lr_angRes_in, Lr_angRes_out] = data_info
        data_info[0] = Lr_angRes_in[0].item()
        data_info[1] = Lr_angRes_out[0].item()
     
        Lr_SAI_y = Lr_SAI_y.to(device)      # low resolution
        Hr_SAI_y = Hr_SAI_y.to(device)    # high resolution

        Sr_4D_y = net(Lr_SAI_y, LFPSF, data_info)
        Sr_4D_y = Sr_4D_y[:,:,0:Hr_SAI_y.shape[2], 0:Hr_SAI_y.shape[3]]
        loss = criterion(Lr_SAI_y, Sr_4D_y, Hr_SAI_y, LFPSF, data_info)
        ...
        loss_iter_test.append(loss.data.cpu())
        ...
        if save_dir is not None and gray:
            ...
            # import ipdb
            # ipdb.set_trace()            
            img = (Sr_SAI_y.squeeze(0).squeeze(0).cpu().detach().numpy().clip(0,1)*255).astype('uint8')
    ...
    return psnr_iter_test, ssim_iter_test, LF_iter_test, float(np.array(loss_iter_test))
