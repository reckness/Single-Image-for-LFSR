...
def parse_args():
    ...
    parser.add_argument("--scale_factor", type=int, default=6, help="4, 2")
    ...
    parser.add_argument('--save_data_path', type=str, default='./datasets/', help='')
    parser.add_argument('--downsample', type=str, default='bicubic', help='')
...
def main(args):
    ...
    for index_dataset in range(len(src_datasets)):
        ...
        # if src_datasets[index_dataset] not in ['DIV2K']:
        #     continue
        idx_save = 0
        ...
        for root, dirs, files in os.walk(src_sub_dataset):
            for file in files:
                ...
                if src_datasets[index_dataset] in ['DIV2K']:
                    LF = LF / 255.0
                ...
                for h in range(0, H - patchsize + 1, stride):
                    for w in range(0, W - patchsize + 1, stride):
                        ...
                        Hr_SAI_y = np.zeros((U * V, patchsize,  patchsize),dtype='single')
                        Lr_SAI_y = np.zeros((U * V,  patchsize // scale_factor, patchsize // scale_factor),dtype='single')
                        ...
                        for u in range(U):
                            for v in range(V):
                                ...
                                Hr_SAI_y[u*U+v, :, :] = tmp_Hr_y

                                # import ipdb
                                # ipdb.set_trace()

                                if args.downsample == 'bicubic':
                                    tmp_Sr_y = imresize(tmp_Hr_y, scalar_scale=downRatio)
                                else:
                                    tmp_Sr_y = tmp_Hr_y[0:tmp_Hr_y.shape[0]:scale_factor, 0:tmp_Hr_y.shape[1]:scale_factor]
