...
def parse_args():
    ...
    parser.add_argument('--dataset',  type=str, default='226-DOF1', help='')
...
def main(args):
    ...
    for index_dataset in range(len(src_datasets)):
        if src_datasets[index_dataset] not in [args.dataset]: # ['EPFL', 'HCI_new', 'HCI_old', 'INRIA_Lytro', 'Stanford_Gantry']:
            continue
        idx_save = 0
            ...
            for file in files:
                ...
                for u in range(U):
                    for v in range(V):
                        ...
                        Lr_SAI_y[u * H: (u+1) * H, v * W: (v+1)* W] = tmp_Lr_y

                        tmp_Hr_y = imresize(tmp_Lr_y, scalar_scale=scale_factor)

                        Hr_SAI_y[u * H * scale_factor : (u+1) * H * scale_factor, v * W * scale_factor : (v+1) * W * scale_factor] = tmp_Hr_y
