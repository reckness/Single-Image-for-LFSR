...
class RCAB(nn.Module):
    def __init__(self, n_feat):
        ...
        self.angular = AngularConv(n_feat)
        self.instance_n = nn.InstanceNorm2d(num_features=n_feat, momentum=0.1)


    def forward(self, x):
    
        return self.body(x) + self.angular(x) + self.instance_n(x)
...
class AngularConv(nn.Module):
    def __init__(self, n_feat):
        ...
        modules_body = [
            nn.Conv2d(n_feat, n_feat//2, 1, 1, 0),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(n_feat//2, 1, 1, 1, 0),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(1, n_feat//2, 1, 1, 0),
            nn.LeakyReLU(0.1, inplace=True),            
            nn.Conv2d(n_feat//2, n_feat, 1, 1, 0),
        ]
        self.body = nn.Sequential(*modules_body)
