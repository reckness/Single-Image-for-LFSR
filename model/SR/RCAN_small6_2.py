...
class get_model(nn.Module):
    def __init__(self, args):
        ...
        modules_upscale2 = Upsampler(conv, 2, n_feats, bn=False, act=False, type='pixelshuffle')
        modules_upscale3 = Upsampler(conv, 3, n_feats, bn=False, act=False, type='pixelshuffle')
        ...
        self.upscale2 = nn.Sequential(*modules_upscale2)
        self.upscale3 = nn.Sequential(*modules_upscale3)
    ...
    def forward(self, input_data, info=None):
        ...
        res = x
        x = self.body(x) + x
        x = self.upscale2(x)  +  F.interpolate(x, scale_factor=2, mode='bicubic', align_corners=False)
        x = self.body(x) + x
        x = self.upscale3(x) +  F.interpolate(x, scale_factor=3, mode='bicubic', align_corners=False)
        out = self.tail(x)
        output = {}
        output['sr'] = out
        output['clean'] = clean
