...
def get_uperleft_denominator(img, kernel):
    ker_f = convert_psf2otf(kernel) # discrete fourier transform of kernel
...
def get_uperleft_denominator_otf(img, ker_f):
    # ker_f = convert_psf2otf(kernel) # discrete fourier transform of kernel
    nsr = wiener_filter_para(img)
    denominator = inv_fft_kernel_est(ker_f, nsr)#
    img1 = img.cuda()
...
# --------------------------------
# --------------------------------
def inv_fft_kernel_est(ker_fc, NSR):
    ker_f = torch.view_as_real(ker_fc)
    # inv_denominator = ker_f * ker_f + NSR
    # # pseudo inverse kernel in flourier domain.
    # inv_ker_f = torch.zeros_like(ker_f)
    # inv_ker_f = ker_f / inv_denominator
    # return inv_ker_f
...
def deconv(inv_ker_f, fft_input_blur):
    ...
    deblur = torch.view_as_real(torch.fft.ifft2(torch.view_as_complex(deblur_f)))
    # deblur = irfft(deblur_f, 3)
    return deblur[:,:,:,:,0]

# --------------------------------
# --------------------------------
def convert_psf2otf(ker):
    ...
    # # otf = rfft(psf, 3)
    # return otf
    return torch.fft.fft2(torch.fft.ifftshift(ker))
    # return torch.fft.fftshift(torch.fft.fft2(torch.fft.ifftshift(ker.cuda())))
