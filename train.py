...
def main(args):
    ...
    # net = torch.compile(net, mode='max-autotune') # torch 2.0
    ...
    for idx_epoch in range(start_epoch, args.epoch):
        ...
        if args.local_rank == 0:
            ...
            save_ckpt_path = str(checkpoints_dir) + '/epoch_%02d_model.pth' % (idx_epoch + 1)
        ...
        '''best epcoh and early-stop'''

        if psnr_epoch_train > psnr_best_train :
            psnr_best_train = psnr_epoch_train
            save_ckpt_path = str(checkpoints_dir) + '/best_model.pth' 
            torch.save(state, save_ckpt_path)

            save_ckpt_path = str(checkpoints_dir) + '/epoch_%02d_model.pth' % (idx_epoch + 1)
            logger.log_string('Saving the best epoch_%02d model at %s' % (idx_epoch + 1, save_ckpt_path))

            ''' Validation real data'''
            cmd = 'python inference_stack.py --model_name {} --angRes {} --scale_factor {} --path_pre_pth {} --test_epoch {} --mark {} --test_data_name {} --angnum {}'.format(args.model_name, args.angRes_in, args.scale_factor, save_ckpt_path, idx_epoch + 1, args.mark, args.test_data_name, args.angnum)
            os.system(cmd) 

        if idx_epoch > 100 and args.early_stop == True :
            if psnr_epoch_train > psnr_early_train:
                psnr_early_train = psnr_epoch_train
                patience_counter = 0 
        ...
        if (idx_epoch + 1)%step==0 or idx_epoch > args.epoch-step:
            with torch.no_grad():
                ...
                for index, test_name in enumerate(test_Names):
                    ...
                    psnr_iter_test, ssim_iter_test, LF_name = test(test_loader, device, net, criterion, save_dir)
...
def train(train_loader, device, net, criterion, optimizer):
    ...
    loss_iter_train = []
    ssim_iter_train = []
    for idx_iter, (input_data, data_info) in tqdm(enumerate(train_loader), total=len(train_loader), ncols=70):
        ...
        out = net(input_data, data_info)
        loss = criterion(input_data, out, data_info)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if isinstance(out, dict):
            ...
            sr = out
        psnr, ssim = cal_metrics(args, label, sr)
...
def test(test_loader, device, net, criterion, save_dir=None):
    LF_iter_test = []
    psnr_iter_test = []
    ssim_iter_test = []
    for idx_iter, (input_data, data_info, LF_name) in tqdm(enumerate(test_loader), total=len(test_loader), ncols=70):
        ...
        Hr_SAI_y = input_data['gt']
        Lr_SAI_y = input_data['input']
        Lr_SAI_y_rs = input_data['restore']
        ...
        if 1:
            ...
            # Sr_SAI_y = Sr_SAI_y[:,:,0:Hr_SAI_y.shape[2], 0:Hr_SAI_y.shape[3]]
        else:
            ''' Crop LFs into Patches '''
            subLFin, numU, numV = Stackdivide(Lr_SAI_y, args.angRes_in, args.patch_size_for_test, args.stride_for_test)
            ...
            ''' Output the Super resolution sub-aperture image''' 
            sai_sr_out = torch.zeros(num, args.angRes_in * args.angRes_in, args.angRes_out * args.patch_size_for_test * args.scale_factor,
                                args.angRes_out * args.patch_size_for_test * args.scale_factor)                     

            tmp_input_data = {}
            tmp_input_data['psf'] = input_data['psf']
            ...
            for i in range(0, num, args.minibatch_for_test):
                ...
                with torch.no_grad():
                    ...
                    out = net(tmp_input_data, data_info)
                    if isinstance(out, dict):
                        sr = out['sr']
                    else:
                        sr = out
                    ...
                    # if isinstance(out, dict):
                    #     if 'sai_sr' in out.keys()
                    #         sai_sr_out[i:min(i + args.minibatch_for_test, num), :, :, :] = out['sai_sr']
        ...
        psnr, ssim = cal_metrics(args, Hr_SAI_y[:,:,12:-12,12:-12], Sr_SAI_y[:,:,12:-12,12:-12])
        ...
        ''' Save Gray '''
        if save_dir is not None and gray:
            save_dir_ = save_dir.joinpath(LF_name[0])
            save_dir_.mkdir(exist_ok=True)

            center_index = Lr_angRes_in * (Lr_angRes_in//2-1) + (Lr_angRes_in//2-1)

            ### save low resolution image
            lr_bicubic = Lr_SAI_y_rs[:,center_index,:,:]
            img = (lr_bicubic.squeeze(0).squeeze(0).cpu().numpy().clip(0,1)*255).astype('uint8')
            path = str(save_dir_) + '/' + 'Lr_'+ LF_name[0] + '.bmp'
            ...
            path = str(save_dir_) + '/' + 'Bic_' + LF_name[0] + '.bmp'
            imageio.imwrite(path, img)

            ### save high resolution image            
            img = (Hr_SAI_y.squeeze(0).squeeze(0).cpu().numpy().clip(0,1)*255).astype('uint8')
            path = str(save_dir_) + '/' + 'Hr_' + LF_name[0] + '.bmp'
            imageio.imwrite(path, img)

            ### save super resolution image
            img = (Sr_SAI_y.squeeze(0).squeeze(0).cpu().numpy().clip(0,1)*255).astype('uint8')
            path = str(save_dir_) + '/' + 'Sr_' + LF_name[0] + '.bmp'
            imageio.imwrite(path, img)
        pass
        ...
        ''' Save RGB '''
        if save_dir is not None and not gray:
            save_dir_ = save_dir.joinpath(LF_name[0])
            save_dir_.mkdir(exist_ok=True)
            ...
            for i in range(args.angRes_out):
                for j in range(args.angRes_out):
                    img = Sr_4D_rgb[i, j, :, :, :]
                    path = str(views_dir) + '/' +  'view_' + str(i) + '_' + str(j) + '.bmp'
                    imageio.imwrite(path, img)
                    pass
                pass
            pass
